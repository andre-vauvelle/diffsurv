...
DATA_DIR = "/Users/andre/Documents/UCL/diffsurv/data/realworld"
RESULTS_DIR = "/Users/andre/Documents/UCL/diffsurv/results"
...
wandb.log(
    {
        "cph_hyperparams": cph_hyperparams,
        "trees_hyperparams": trees_hyperparams
    }
)
...
for dataset in datasets:
    ...
    for outer_train_index, test_index in tqdm(outer_kf.split(X), desc="Outer loop", total=outer_k, ncols=100):
        ...
        X_train, X_val, T_train, T_val, E_train, E_val = train_test_split(X_outer_train, T_outer_train, E_outer_train,
                                                                          test_size=val_split, shuffle=True,
                                                                          random_state=42)
        ...
        for penalizer in tqdm(cph_hyperparams["penalizer"], desc="CPH hyperparams", ncols=100):
            train_df = pd.DataFrame(np.column_stack((X_train, E_train, T_train)), columns=list(range(X_train.shape[1])) + ["event", "time"])
            val_df = pd.DataFrame(np.column_stack((X_val, E_val, T_val)), columns=list(range(X_val.shape[1])) + ["event", "time"])
        ...
        for num_trees, max_depth, min_node_size in tqdm(itertools.product(
                trees_hyperparams["num_trees"],
                trees_hyperparams["max_depth"],
                trees_hyperparams["min_node_size"],
        ), desc="Trees hyperparams", ncols=100,
                total=len(trees_hyperparams["num_trees"]) * len(
                    trees_hyperparams["max_depth"]) * len(trees_hyperparams["min_node_size"])):
        ...
        train_df = pd.DataFrame(np.column_stack((X_outer_train, E_outer_train, T_outer_train)), columns=list(range(X_outer_train.shape[1])) + ["event", "time"])
        test_df = pd.DataFrame(np.column_stack((X_test, E_test, T_test)), columns=list(range(X_test.shape[1])) + ["event", "time"])
        ...
        # Survival Trees
        trees = RandomSurvivalForestModel(**best_trees_params)
        trees.fit(X=X_outer_train, T=T_outer_train, E=E_outer_train)
