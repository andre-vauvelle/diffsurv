...
class BertBase(Bert.modeling.BertPreTrainedModel, BaseModel):
    ...
    def configure_optimizers(self):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in not_sparse.items() if not any(nd in n for nd in no_decay)],
                "weight_decay": 0.01,
            },
            {
                "params": [p for n, p in not_sparse.items() if any(nd in n for nd in no_decay)],
                "weight_decay": 0,
            },
        ]
        optimizer_sparse = torch.optim.SparseAdam(sparse.values(), lr=self.lr)
        optimizer = Bert.optimization.BertAdam(
            optimizer_grouped_parameters, lr=self.lr, warmup=self.warmup_proportion
        )
...
class BERTMLM(Bert.modeling.BertPreTrainedModel, pl.LightningModule):
    ...
    def __init__(
        ...
        self.head = CustomBertLMPredictionHead(
