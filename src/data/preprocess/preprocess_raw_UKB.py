...
from src.omni.common import load_pickle, save_pickle
...
def config():
    ...
    gp_prescriptions_data_path = (
        os.path.join(DATA_DIR, "raw", "application58356", "gp_scripts.tsv"),
    )
    # External data (None patient sensitive)
    caliber_secondary_care_dict_path = (
        os.path.join(EXTERNAL_DATA_DIR, "caliber_secondary_care_dict.csv"),
    )
...
class BiobankDataset:
    def __init__(
        ...
        opcs_event_data_path=os.path.join(DATA_DIR, "raw", "application58356", "hesin_oper.tsv"),
        gp_event_data_path=os.path.join(DATA_DIR, "raw", "application58356", "gp_clinical.tsv"),
        gp_prescriptions_data_path=os.path.join(
            DATA_DIR, "interim", "codes_gp_prescription_211124.feather"
        ),
        # External data (None patient sensitive)
        caliber_secondary_care_dict_path=os.path.join(
            EXTERNAL_DATA_DIR, "caliber_secondary_care_dict.csv"
        ),
        icd10_phecode=os.path.join(EXTERNAL_DATA_DIR, "phecode_icd10.csv"),
        read2_phecode=os.path.join(EXTERNAL_DATA_DIR, "read2_to_phecode.csv"),
        readctv3_phecode=os.path.join(EXTERNAL_DATA_DIR, "readctv3_to_phecode.csv"),
    ...
    def get_patient_events(self, patient_base) -> pd.DataFrame:
        ...
        diag_data = pd.read_csv(
            self.diag_event_data_path, delimiter="\t"
        )  # raw data doesn't have dots for icd10
        ...
        deaths = pd.read_csv(
            os.path.join(DATA_DIR, "raw", "application58356", "death.txt"), delimiter="\t"
        )
        death_cause = pd.read_csv(
            os.path.join(DATA_DIR, "raw", "application58356", "death_cause.txt"), delimiter="\t"
        )
        ...
        diag_data = pd.merge(
            diag_data, event_data, on=id_vars
        )  # diag_data = diag_data[diag_data.level != 3]
        diag_data = diag_data[
            diag_data.level != 3
        ]  # remove external diagnosis codes (only 2% of total)

        deaths = deaths.merge(death_cause, how="inner", on="eid")

        # massage into one df
        desired_columns = ["eid", "date", "code_type", "code"]
        diag_data.loc[:, "code"] = diag_data.diag_icd10.str[
            :MAX_CODE_LENGTH
        ]  # remove extra subsub chapter details
        ...
        diag_data.loc[:, "code_type"] = diag_data.level.map(
            {1: "diag_primary", 2: "diag_secondary"}
        )
        opcs_data.loc[:, "code_type"] = opcs_data.level.map(
            {1: "oper_primary", 2: "oper_secondary"}
        )
        ...
        patient_event_data = pd.concat(
            [diag_data, opcs_data, gp_clinical, gp_scripts, deaths, deaths_nocause], axis=0
        )

        patient_event_data = pd.merge(
            patient_event_data, patient_base.loc[:, ["eid", "yob"]], how="inner", on="eid"
        )
    ...
    def caliber_convert(self, patient_event_data):
        ...
        icd_caliber_dict = {
            k: v for k, v in zip(secondary_caliber.ICD10code, secondary_caliber.Disease)
        }
        opcs_caliber_dict = {
            k: v for k, v in zip(secondary_caliber.OPCS4code, secondary_caliber.Disease)
        }

        patient_event_data.loc[:, "caliber"] = ""
        # match of chapter first
        patient_event_data.loc[patient_event_data.code_type == "diag_icd10", "caliber"] = (
            patient_event_data[patient_event_data.code_type == "diag_icd10"]
            .code.str[:3]
            .map(icd_caliber_dict)
        )
        # then sub chapter
        patient_event_data.loc[
            patient_event_data.code_type == "diag_icd10", "caliber"
        ] = patient_event_data[patient_event_data.code_type == "diag_icd10"].code.map(
            icd_caliber_dict
        )
        patient_event_data.loc[
            patient_event_data.code_type == "secondary_icd10", "caliber"
        ] = patient_event_data[patient_event_data.code_type == "secondary_icd10"].code.map(
            icd_caliber_dict
        )
        patient_event_data.loc[
            patient_event_data.code_type == "oper4", "caliber"
        ] = patient_event_data[patient_event_data.code_type == "oper4"].code.map(opcs_caliber_dict)
        return patient_event_data
    ...
    def phecode_convert(self, patient_event_data, many_to_one=True):
        ...
        bool_diag = patient_event_data.code_type.str.contains(
            r"diag_secondary|diag_primary|diag_death", regex=True
        )
        patient_event_data.loc[bool_diag, "code_type_match"] = "diag"
        patient_event_data.loc[bool_diag, "code"] = patient_event_data.loc[bool_diag, "code"].str[
            :4
        ]

        patient_event_data = pd.merge(
            patient_event_data, phecode_lookup, on=["code", "code_type_match"], how="left"
        )
    ...
    def omop_covert(
        ...
        def merge_process(chunk):
            ...
            a = chunk.query(
                "code_type_match == 'read' or code_type_match == 'gp'"
            )  # HACK to fix above
            idx = a.index
            values = a.code_type
        ...
        if patient_event_data_path is not None:
            ...
            patient_event_data = pd.concat(
                [
                    merge_process(chunk)
                    for chunk in tqdm(ped, total=int(206766864 / chunksize), desc="merging omop id")
                ],
                axis=0,
            )
            del ped
        else:
            patient_event_data.loc[:, "code_type_match"] = patient_event_data.code_type.str.split(
                "_"
            ).str[0]
            a = patient_event_data.query("code_type_match == 'read'")
            idx = a.index
            values = a.code_type
            patient_event_data.loc[idx, "code_type_match"] = values
            patient_event_data = patient_event_data.merge(
                omop_map, on=["code", "code_type_match"], how="left"
            )
    ...
    def add_separator(
        patient_event_data,
        sep_vars=("eid", "date"),
        code_cols=("code", "phecode", "concept_id"),
        ...
        patient_event_data.loc[
            :, "sorting_var"
        ] = 0  # used to ensure token at the end of every date
        ...
        patient_event_data.phecode = patient_event_data.phecode.cat.add_categories(
            [separator_token]
        )
        patient_event_data.concept_id = patient_event_data.concept_id.cat.add_categories(
            [separator_token]
        )
    ...
    def get_phe_data(self, patient_event_data=None, reload=True):
        ...
        if reload:
            ...
            patient_event_data = pd.read_csv(
                os.path.join(DATA_DIR, "interim", "patient_event_data.csv"),
                dtype=d_types,
                parse_dates=["date"],
            )
    ...
    def phe_lists(self, with_codes, reload=True):
        if reload:
            ...
            save_pickle(
                code_vocab, os.path.join(DATA_DIR, "processed", with_codes, "code_vocab.pkl")
            )
            phe_lists = self.get_sequences_df(phe_data)
            return phe_lists, age_vocab, code_vocab
        if not reload:
            phe_lists = pd.read_csv(
                os.path.join(DATA_DIR, "processed", with_codes, "phe_lists.csv")
            )
            phe_lists.age = phe_lists.age.apply(literal_eval)
            phe_lists.phecode = phe_lists.phecode.apply(literal_eval)
            age_vocab = load_pickle(
                os.path.join(DATA_DIR, "processed", with_codes, "age_vocab.pkl")
            )
            code_vocab = load_pickle(
                os.path.join(DATA_DIR, "processed", with_codes, "code_vocab.pkl")
            )
            return phe_lists, age_vocab, code_vocab
...
def train_val_test(df_input, stratify_colname=None, frac_train=0.6, frac_val=0.2, frac_test=0.2):
    ...
    if frac_train + frac_val + frac_test != 1.0:
        raise ValueError(
            "fractions {:f}, {:f}, {:f} do not add up to 1.0".format(
    ...
    df_train, df_temp, y_train, y_temp = train_test_split(
        X, y, stratify=y, test_size=(1.0 - frac_train)
    )

    # Split the temp dataframe into val and test dataframes.
    relative_frac_test = frac_test / (frac_val + frac_test)
    df_val, df_test, y_val, y_test = train_test_split(
        df_temp, y_temp, stratify=y_temp, test_size=relative_frac_test
    )
...
def main(
    ...
    raw_patient_table.to_feather(
        os.path.join(DATA_DIR, "processed", "covariates", "eid_covariates.feather")
    )
    raw_patient_table = pd.read_feather(
        os.path.join(DATA_DIR, "processed", "covariates", "eid_covariates.feather")
    )

    print(f"Raw. Total patients: {raw_patient_table.eid.unique().shape[0]}")
    ...
    if patient_event_data is None:
        patient_event_data = b.omop_covert(
            patient_event_data_path=patient_event_data_path, chunksize=10_000
        )
    else:
        print(
            "Within HES or primary care data. Total patients: {}, Total events: {}".format(
                patient_event_data.eid.unique().shape[0], patient_event_data.shape[0]
            )
        )
        patient_event_data = b.omop_covert(patient_event_data=patient_event_data)

    print(
        "After omop convert. Total patients: {}, Total events: {}".format(
            patient_event_data.eid.unique().shape[0], patient_event_data.shape[0]
        )
    )
    ...
    print(
        "After phecode convert. Total patients: {}, Total events: {}".format(
            patient_event_data.eid.unique().shape[0], patient_event_data.shape[0]
        )
    )

    patient_event_data = patient_event_data[
        (patient_event_data.date.dt.year > 1950) & (patient_event_data.date.dt.year < 2021)
    ]
    print(
        "Within Study period. Total patients: {}, Total events: {}".format(
            patient_event_data.eid.unique().shape[0], patient_event_data.shape[0]
        )
    )
    ...
    phe_lists["length"] = phe_lists["concept_id"].apply(lambda x: len(x))
    ...
    phe_train, phe_val, phe_test = train_val_test(
        phe_lists, frac_train=0.8, frac_val=0.1, frac_test=0.1
    )
    ...
    phecode_vocab_top = fit_vocab(
        phe_train.phecode.explode().astype(str),
        min_proportion=0.01,
    )
    ...
    save_pickle(
        age_vocab, os.path.join(DATA_DIR, "processed", with_codes, f"age_vocab_{age_vocab_len}.pkl")
    )
    save_pickle(
        code_vocab,
        os.path.join(DATA_DIR, "processed", with_codes, f"code_vocab_{code_vocab_len}.pkl"),
    )
    save_pickle(
        phecode_vocab,
        os.path.join(DATA_DIR, "processed", with_codes, f"phecode_vocab_{phecode_vocab_len}.pkl"),
    )
    save_pickle(
        phecode_vocab_top,
        os.path.join(
            DATA_DIR, "processed", with_codes, f"phecode_vocab_top_{phecode_vocab_top_len}.pkl"
        ),
    )
    save_pickle(
        concept_vocab,
        os.path.join(DATA_DIR, "processed", with_codes, f"concept_vocab_{concept_vocab_len}.pkl"),
    )
    save_pickle(
        concept_vocab_15k,
        os.path.join(
            DATA_DIR, "processed", with_codes, f"concept_vocab_{concept_vocab_15k_len}.pkl"
        ),
    )
    ...
    for df, name in zip(
        [phe_train, phe_val, phe_test], ["phe_train.feather", "phe_val.feather", "phe_test.feather"]
    ):
        ...
        print(
            f"mlm {name}. Total patients {df.shape[0]}, Total events"
            f" {df.phecode.explode().shape[0]}"
        )
