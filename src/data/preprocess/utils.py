...
def get_icd_omop():
    ...
    icd10_omop = (
        vocab["concept"]
        .query("vocabulary_id in ('ICD10')")[
            ["concept_code", "concept_id", "concept_name", "vocabulary_id"]
        ]
        .rename(columns={"concept_code": "code"})
    )
    # icd10_omop = vocab["concept"][
    #     ["concept_code", "concept_id", "concept_name",'vocabulary_id']].rename(columns={"concept_code": "code"})
    icd10_omop["code"] = icd10_omop["code"].replace(r"\.", "", regex=True)
    ...
    origin_codes = (
        vocab["concept"]
        .query("vocabulary_id==@origin")
        .query("domain_id==@domains")
        .concept_id.to_list()
    )
    target_codes = (
        vocab["concept"]
        .query("vocabulary_id==@target&(standard_concept=='S'|standard_concept=='C')")
        .query("domain_id==@domains")
        .concept_id.to_list()
    )
    snomed_map = (
        ...
        .query(
            "concept_id_1 == @origin_codes & concept_id_2 == @target_codes &"
    ...
    icd10_omop = icd10_omop.merge(
        snomed_map, left_on="concept_id_icd", right_on="concept_id_origin", how="left"
    )
    ...
    icd10_omop.concept_class_id = icd10_omop.concept_class_id.astype(
        "category"
    ...
    icd10_omop = icd10_omop.sort_values(by="concept_class_id").drop_duplicates(
...
def get_opcs_omop():
    ...
    #     ["concept_code", "concept_id", "concept_name",'vocabulary_id']].rename(columns={"concept_code": "code"})
    opcs_omop["code"] = opcs_omop["code"].replace(r"\.", "", regex=True)
    ...
    opcs_omop.concept_class_id = opcs_omop.concept_class_id.astype(
        "category"
    ).cat.reorder_categories(new_categories=top_class, ordered=True)
    opcs_omop = opcs_omop.sort_values(by="concept_class_id").drop_duplicates(
...
def get_phecode_map():
    ...
    phecode_lookup.phecode = phecode_lookup.phecode.str.replace(r"\.0", "", regex=False)
...
def get_read_omop():
    ...
    rc2_map = pd.read_csv(
        os.path.join(
            mapping_tables_dir, "Updated/Not Clinically Assured/rcmap_uk_20200401000001.txt"
        ),
        sep="\t",
    )
    rc3_map = pd.read_csv(
        os.path.join(
            mapping_tables_dir, "Updated/Clinically Assured/ctv3sctmap2_uk_20200401000001.txt"
        ),
        sep="\t",
    )
...
def update_with_phecode_x(phe_data):
    ...
    sno_icd_phe_map = pd.read_feather(
        os.path.join(EXTERNAL_DATA_DIR, "snomed_icd10_phecode_mapping_220407.feather")
    )
    ...
    phe_data_x = phe_data.merge(
        sno_icd_phe_map.loc[:, ["concept_id", "phecode_X", "sex"]], on=["concept_id"], how="left"
    )
    ...
    phe_data_x_deaths = phe_data_x.merge(
        deaths.loc[:, ["eid", "date"]], on=["eid", "date"], how="inner"
    )
    ...
    phe_data_x.to_csv(
        "/SAN/ihibiobank/denaxaslab/andre/UKBB/data/processed/omop/phe_data_x.csv", index=False
    )
    phe_data_x.to_feather(
        "/SAN/ihibiobank/denaxaslab/andre/UKBB/data/processed/omop/phe_data_x.feather"
    )
    return phe_data_x
...
def align_pretrained_embedding(embedding, token2idx, save_path):
    ...
    rand_embeddings = np.random.rand(
        token2idx_df.shape[0], found_embeddings.embedding.iloc[0].shape[0]
    )

    # Fill in found values
    rand_embeddings[found_embeddings.index] = np.concatenate(
        found_embeddings.embedding.values, axis=0
    ).reshape(found_embeddings.shape[0], -1)
...
def vocab_omop_embedding(
    embedding,
    token2idx,
    save_path="/SAN/ihibiobank/denaxaslab/andre/ehrgraphs/models/embeddings/default.pkl",
) -> torch.Tensor:
    """
    ...
    token2idx_df.tokens = token2idx_df.tokens.str.replace(r"\.", "", regex=True)
    ...
    # Get omop codes
    token2idx_df = token2idx_df.merge(
        omop_map.loc[:, ["code", "concept_id"]], left_on="tokens", right_on="code", how="left"
    )
    # token2idx_df.concept_id.notna().sum()
    # Get embedding arrays
    token2idx_df = token2idx_df.merge(
        embedding, on="concept_id", how="left", validate="many_to_one"
    )
    ...
    embedding_tensor[pretrained_idxs] = torch.from_numpy(
        np.stack(matched_embeddings.values)
    ).float()
...
def fit_vocab(data: List, min_count=None, min_proportion=None, top_n=None, label=False) -> Dict:
    ...
    counts.drop(list(SYMBOL_IDX.values()), inplace=True, errors="ignore")
    ...
    if min_count is not None:
        ...
        print(
            f"Excluding {proportions[proportions < min_proportion].sum()} tokens with proportion <"
            f" {min_proportion}"
        )
    ...
    vocab = {"idx2token": idx2token, "token2idx": token2idx}
    return vocab
