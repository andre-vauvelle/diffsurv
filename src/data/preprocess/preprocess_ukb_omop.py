...
from data.preprocess.utils import (
    align_pretrained_embedding,
    reformat_gnn_embedding,
    vocab_omop_embedding,
)
from definitions import DATA_DIR
from omni.common import load_pickle

if __name__ == "__main__":
    code_vocab_path = os.path.join(DATA_DIR, "processed", "omop", "concept_vocab_4809.pkl")
    # code_vocab_path = os.path.join(DATA_DIR, 'processed', 'in_gnn', 'concept_vocab_1561.pkl')
    in_gnn = False

    embeddings_path_store = [
        "/SAN/ihibiobank/denaxaslab/andre/ehrgraphs/models/embeddings/gnn_embeddings_256_1gr128qk_20220217.feather",
        "/SAN/ihibiobank/denaxaslab/andre/ehrgraphs/models/embeddings/graph_full_211122_prone_32_edge_weights_2021-12-13.feather",
        "/SAN/ihibiobank/denaxaslab/andre/ehrgraphs/models/embeddings/graph_full_211209_prone_256_edge_weights_no_shortcuts_2022-01-05.feather",
    ]

    for embeddings_path in embeddings_path_store:
        save_path = (
            embeddings_path[:-8] + ".pt" if not in_gnn else embeddings_path[:-8] + "_in_gnn.pt"
        )
        ...
        embedding_tensor, missing_embeddings = align_pretrained_embedding(
            embedding, token2idx, save_path
        )
